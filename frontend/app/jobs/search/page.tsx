...
/* Removed user-specific queries for static conversion */
...
function SearchResults() {
  ...
  const { jobs, loading, error, fetchJobs } = useJobSearch();
  ...
  // Filter jobs based on client-side filters (Job Type, Date)
  const filteredJobs = React.useMemo(() => {
    return jobs.filter((job) => {
      // Location Filter (Refinement)
      if (localLocation) {
        const l = localLocation.toLowerCase();
      ...
      if (selectedDatePosted) {
        ...
        if (selectedDatePosted === "Last 24 hours" && diffDays > 1) return false;
        if (selectedDatePosted === "Last 3 days" && diffDays > 3) return false;
        if (selectedDatePosted === "Last 7 days" && diffDays > 7) return false;
        if (selectedDatePosted === "Last 14 days" && diffDays > 14) return false;
      }

      return true;
    });
  }, [jobs, localLocation, selectedJobTypes, selectedDatePosted]);
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-[var(--blue-darkest)] via-[var(--blue-dark)] to-[var(--blue-medium-dark)] flex items-center justify-center">
        <div className="text-white text-xl animate-pulse">Searching for jobs...</div>
      </div>
    );
  }
